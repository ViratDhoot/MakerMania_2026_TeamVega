...
void drawMaze(uint8_t maze[MAZE_W][MAZE_H], int ix, int iy) {
  ...
  for (int i = max((ix / DIM[0]) - 6, 0); i < min(MAZE_W, (ix / DIM[0]) + 6); i++) {
    for (int j = max((iy / DIM[1]) - 7, 0); j < min(MAZE_H, (iy / DIM[1]) + 7); j++) {
      ...
      if (i == MAZE_W - 1)
        display.gfx().drawLine(cx + DIM[0] - 1, cy, cx + DIM[0] - 1, cy + DIM[1], SWHITE);
      if (j == MAZE_H - 1)
        display.gfx().drawLine(cx, cy + DIM[1] - 1, cx + DIM[0], cy + DIM[1] - 1, SWHITE);
    }
  }
}
  ...
  if (errEvent.type != NO_ERR && millis() - errEvent.shownAt > errEvent.duration) {
    ...
    switch (currFrame) {
      ...
      case GAME:
        ...
        drawMaze(maze, net._me.x, net._me.y);
        ...
        for (int i = 0; i < NUM_PLAYERS - 1; i++) {
          ...
          int dx = net._me.x - SCREEN_WIDTH / 2;
          int dy = net._me.y - SCREEN_HEIGHT / 2;
